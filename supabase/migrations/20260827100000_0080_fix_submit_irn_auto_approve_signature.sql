-- submit_irn's internal Admin/Team-Lead auto-approve shortcut still called
-- approve_irn(uuid, uuid) after 0074 changed approve_irn to require a
-- signature — crashing every self-approved material inward with
-- "function public.approve_irn(uuid, uuid) does not exist". Add an optional
-- p_signature_id and only attempt the inline approval when one is supplied
-- (best-effort, same silent-no-op pattern as createGrn's auto-sign); with no
-- signature (or if approve_irn itself errors) the IRN simply falls through to
-- pending_approval instead of crashing the whole submission.

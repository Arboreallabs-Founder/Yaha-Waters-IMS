-- ============================================================
-- 0057 — Allow IRN inspection templates on box-tracked components.
--
-- The original exclusion (0036) assumed "a box represents many pieces
-- sharing one QR, one IRN's answers can't distinguish them" — but a
-- freshly-received box is mechanically identical to bulk: one grn_lines
-- row -> one lot/QR (grn_line_after_insert() branch (c)), so IRN's existing
-- "defer the grn_lines insert until approval" pattern already applies
-- cleanly. The one real gap was the "add pieces to an existing open box"
-- shortcut (grn_lines.target_lot_id, branch (a) of the same trigger) —
-- IRN had nowhere to carry that choice through approval. This migration
-- adds it, so all three tracking modes now go through the exact same
-- deferred-until-approved mechanism.
-- ============================================================

-- ---- 1) components: box may now be inspection-gated -----------------------
alter table public.components drop constraint chk_components_irn_not_box;

-- ---- 2) irns: carry the "add to this existing box" choice through approval
alter table public.irns add column target_lot_id uuid references public.inventory_lots(id);

-- ---- 3) submit_irn: accept target_lot_id, drop the box hard-block ---------
drop function if exists public.submit_irn(uuid,uuid,numeric,numeric,uuid,uuid,numeric,numeric,numeric,jsonb,uuid);
...
begin
  ...
  insert into public.irns(irn_no, grn_id, component_id, template_id, qty, unit_cost, po_line_id, project_id,
        piece_count, piece_length, piece_width, target_lot_id, generated_by, created_by)
  values (v_pno, p_grn_id, p_component_id, v_template, p_qty, p_unit_cost, p_po_line_id, p_project_id,
        p_piece_count, p_piece_length, p_piece_width, p_target_lot_id, p_submitter_id, p_submitter_id)
...
grant execute on function public.submit_irn(uuid,uuid,numeric,numeric,uuid,uuid,numeric,numeric,numeric,jsonb,uuid,uuid) to authenticated;

-- ---- 4) approve_irn: carry target_lot_id into the deferred grn_lines insert
...
-- ---- 5) resubmit_irn: carry target_lot_id through the clone --------------
...
begin
  ...
  v_result := public.submit_irn(v_old.grn_id, v_old.component_id, v_old.qty, v_old.unit_cost,
    ...
    p_answers, p_submitter_id, v_old.target_lot_id);

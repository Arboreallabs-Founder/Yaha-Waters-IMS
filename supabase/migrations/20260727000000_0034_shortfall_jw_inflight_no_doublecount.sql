-- ============================================================
-- 0034 — project_shortfall() stops double-counting job-work stock.
--
-- Previously, the "already consumed" credit summed ALL project-tagged
-- issue movements with no reference_type filter. dispatch_job_work()
-- writes such an issue movement when raw stock is sent to a JW vendor
-- (reference_type='job_work') — correct while genuinely in-flight. But
-- once receive_job_work() creates the new completed-stage lot, that
-- lot's on-hand is *also* counted, while the original dispatch credit
-- is a permanent ledger row that never goes away. Net effect: the same
-- physical units get credited twice after a full dispatch->receive
-- cycle, making shortfall_qty too low (under-reports real shortfall).
--
-- Fix: split the credit into
--   (a) genuine consumption — issue movements NOT tagged job_work
--   (b) in-flight-to-JW — sum(qty_sent - qty_returned) from
--       job_work_lines/job_work_orders directly, which naturally
--       shrinks to 0 as qty_returned catches up to qty_sent.
-- Adds a new sent_to_jw_qty display column; consumed_qty keeps its
-- existing "genuinely consumed" meaning.
-- ============================================================
...
returns table(
  ...
  sent_to_jw_qty numeric,
  shortfall_qty numeric
)
language plpgsql stable security definer set search_path = public as $$
#variable_conflict use_column
declare
  v_stock jsonb := '{}';    -- component_id::text -> working pool = on-hand + consumed + jw-in-flight (this project); mutated during the walk
  v_onhand jsonb := '{}';   -- component_id::text -> physical on-hand only (display column, immutable)
  v_consumed jsonb := '{}'; -- component_id::text -> genuinely consumed for this project (display column, immutable; excludes job-work dispatch)
  v_sentjw jsonb := '{}';   -- component_id::text -> qty currently in-flight to a JW vendor for this project (display column, immutable)
...
begin
  ...
  -- Genuine consumption only: excludes the job-work dispatch issue movement,
  -- which is credited separately below (and shrinks to 0 on full return).
  ...
  -- In-flight to job-work: derived from job_work_lines/job_work_orders state
  -- directly (not the ledger), so it self-corrects as qty_returned catches
  -- up with qty_sent — no reliance on the dispatch movement ever "expiring".
  select coalesce(jsonb_object_agg(t.cid::text, t.sq), '{}') into v_sentjw
  from (select jwl.component_id cid, sum(greatest(jwl.qty_sent - jwl.qty_returned, 0)) sq
        from public.job_work_lines jwl
        join public.job_work_orders jwo on jwo.id = jwl.jw_order_id
        where jwo.project_id = p_project and jwo.status in ('sent','partial')
          and jwl.component_id is not null
        group by jwl.component_id) t;
  ...
  from (
    select cid, sum(amt) amt from (
      ...
      union all
      select jwl.component_id cid, sum(greatest(jwl.qty_sent - jwl.qty_returned, 0)) amt
      from public.job_work_lines jwl
      join public.job_work_orders jwo on jwo.id = jwl.jw_order_id
      where jwo.project_id = p_project and jwo.status in ('sent','partial')
        and jwl.component_id is not null
      group by jwl.component_id
    ) x group by cid
  ) t;
  ...
  select p_project,
         ...
         coalesce((v_sentjw->>e.key)::numeric, 0),

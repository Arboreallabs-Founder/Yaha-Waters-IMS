...
type ChecklistEntry = { label: string; field_type: string; value: string | null };
type Irn = {
...
type Traceability = { lot: Lot; lineage: LineageEntry[]; purchase_order: PurchaseOrder | null; grn: Grn | null; job_work: JobWork[]; irn: Irn[]; movements: Movement[] };
            ...
            <CardContent className="p-5">
              <div className="mb-3 flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">
                  Inspection (IRN){result.irn.length > 1 ? ` — ${result.irn.length} entries` : ""}
                </h3>
              </div>
              {result.irn.length === 0 ? (
